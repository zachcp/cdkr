...
Front a molecule, defined as conjunct of atoms held together by chemical
bonds, we can simplify it taking only the information about the atoms. The `rcdk`
package provides a parser to translate molecules to molecular formulas, the
...
isvalid.formula(formula,rule=c("nitrogen","RDBE"))
```
The `generate.formula` method can perform these validation tests during formula generation by setting `validation=TRUE`. However, this can significantly slow down the process of generating formulae, especially with larger mass windows. When the default of `FALSE` is used, nonsensical formulae may be generated, and it's up to the user to filter them out.
